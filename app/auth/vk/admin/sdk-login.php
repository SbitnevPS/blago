...
$redirect = (string) ($body['redirect'] ?? ($_POST['redirect'] ?? '/admin'));
$vkPayload = $body['vk'] ?? [];
...
$flowData = vkid_sdk_flow_consume('admin', $state);
...
$exchange = vkid_exchange_sdk_code_for_tokens($code, $deviceId, $codeVerifier, VK_ADMIN_REDIRECT_URI, $state);
...
$result = vk_admin_login_by_access_token($pdo, $accessToken, $redirect, $vkUserId, $vkEmail, $idToken, $refreshToken, $claims);
if (empty($result['ok'])) {
    jsonResponse([
        'success' => false,
...
jsonResponse([
    'success' => true,
    'redirect_to' => $result['redirect_to'] ?? '/admin',
]);
