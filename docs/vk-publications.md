...
## Публикация работ в VK: источник токена

Публикация может работать в одном из двух режимов (выбирается в `/admin/settings`):

- **VK ID токен из сессии админа** (user access_token, вход через VK ID SDK).
- **Ключ доступа сообщества** (community/group access token, сохранённый в настройках).

Важно: публикация использует **только один** выбранный источник токена, без смешивания и скрытых fallback.

## Ограничения режима community token

Текущая реализация публикации работ использует upload-chain для загрузки нового локального изображения:
...
Этот сценарий является **user-only** и **не поддерживается** при работе через community token. В режиме community token публикация работ (с загрузкой изображения с диска) блокируется понятной ошибкой и не стартует.

Сценарий публикации использует **user access_token**, полученный при входе администратора через **VK ID SDK**.
Токен хранится в PHP-сессии и живёт, пока активна сессия (при необходимости обновляется через refresh_token).
Готовность публикации проверяется через `POST /auth/vk/publication/test`.

### Какие настройки используются

В системных настройках (`storage/settings.json`) сохраняются:

- токен **не хранится** в настройках (только в сессии админа);
...
Если VK возвращает `method is unavailable with group auth`, это означает, что используется токен сообщества (group auth).
В текущей схеме должен использоваться **user token из VK ID сессии админа**: перелогиньтесь в админку через VK ID.
